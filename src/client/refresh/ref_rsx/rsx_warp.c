...
static void
R_RSX_Warp_boundPoly(int numverts, float *verts, vec3_t mins, vec3_t maxs)
{
	int i, j;
	float *v;
...
static void
R_RSX_Warp_SubdividePolygon(int numverts, float *verts, msurface_t *warpface)
{
	int i, j, k;
	vec3_t mins, maxs;
	float m;
	...
	R_RSX_Warp_boundPoly(numverts, verts, mins, maxs);
	...
	for (i = 0; i < 3; i++)
	{
		...
		R_RSX_Warp_SubdividePolygon(f, front[0], warpface);
		R_RSX_Warp_SubdividePolygon(b, back[0], warpface);
		return;
	}
 ...
 */
void
R_RSX_Warp_SubdivideSurface(msurface_t *fa, gl3model_t* loadmodel)
{
	vec3_t verts[64];
	int numverts;
	int i;
	...
	R_RSX_Warp_SubdividePolygon(numverts, verts[0], fa);
}
 ...
 */
void
R_RSX_Warp_EmitWaterPolys(msurface_t *fa)
{
	glpoly_t *bp;
	float scroll = 0.0f;
	...
	R_RSX_Shaders_Load(&(gl3state.fs3Dwater));
	...
	for (bp = fa->polys; bp != NULL; bp = bp->next)
	{
		...
		R_RSX_BufferAndDraw3D(bp->vertices, bp->numverts, GCM_TYPE_TRIANGLE_FAN);
	}
}
...
//313
void
R_RSX_Warp_SetSky(char *name, float rotate, vec3_t axis)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		sky_images[i] = R_RSX_Image_FindImage(pathname, it_sky);

		if (sky_images[i] == NULL || sky_images[i] == rsx_no_texture)
		{
...
static void
R_RSX_Warp_drawSkyPolygon(int nump, vec3_t vecs)
{
	int i, j;
	vec3_t v, av;
	float s, t, dv;
...
static void
R_RSX_Warp_clipSkyPolygon(int nump, vec3_t vecs, int stage)
{
	float *norm;
	float *v;
	qboolean front, back;
	...
	{
		ri.Sys_Error(ERR_DROP, "R_R_RSX_Warp_clipSkyPolygon: MAX_CLIP_VERTS");
	}

	if (stage == 6)
	{
		/* fully clipped, so draw it */
		R_RSX_Warp_drawSkyPolygon(nump, vecs);
		return;
	}
	...
	{
		/* not clipped */
		R_RSX_Warp_clipSkyPolygon(nump, vecs, stage + 1);
		return;
	}
	...
	/* continue */
	R_RSX_Warp_clipSkyPolygon(newc[0], newv[0][0], stage + 1);
	R_RSX_Warp_clipSkyPolygon(newc[1], newv[1][0], stage + 1);
}

void
R_RSX_Warp_AddSkySurface(msurface_t *fa)
{
	// static qboolean is_first_call = true;
	// if (is_first_call)
	// {
	...
	for (p = fa->polys; p; p = p->next)
	{
		...
		R_RSX_Warp_clipSkyPolygon(p->numverts, verts[0], 0);
	}
}

void // 596
R_RSX_Warp_ClearSkyBox(void)
{
	// static qboolean is_first_call = true;
	// if (is_first_call)
	// {
...
static void
R_RSX_Warp_makeSkyVec(float s, float t, int axis, gl3_3D_vtx_t* vert)
{
	vec3_t v, b;
	int j, k;
...
R_RSX_Warp_DrawSkyBox(void)
{
	...
	gl3state.uni3DData.transModelMat4 = HMM_Transpose(modMVmat);
	// GL3_UpdateUBO3D();
	R_RSX_Shaders_UpdateUni3DData();
	...
	for (i = 0; i < 6; i++)
	{
		...
		R_RSX_Image_BindUni2DTex0(sky_images[skytexorder[i]]);
		R_RSX_Shaders_Load(&(gl3state.fs3Dsky));

		R_RSX_Warp_makeSkyVec( skymins [ 0 ] [ i ], skymins [ 1 ] [ i ], i, &skyVertices[0] );
		R_RSX_Warp_makeSkyVec( skymins [ 0 ] [ i ], skymaxs [ 1 ] [ i ], i, &skyVertices[1] );
		R_RSX_Warp_makeSkyVec( skymaxs [ 0 ] [ i ], skymaxs [ 1 ] [ i ], i, &skyVertices[2] );
		R_RSX_Warp_makeSkyVec( skymaxs [ 0 ] [ i ], skymins [ 1 ] [ i ], i, &skyVertices[3] );

		// GL3_BufferAndDraw3D(skyVertices, 4, GL_TRIANGLE_FAN);
		R_RSX_BufferAndDraw3D(skyVertices, 4, GCM_TYPE_TRIANGLE_FAN);
	}
